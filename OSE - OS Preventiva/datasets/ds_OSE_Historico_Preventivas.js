...
function getQuery(LOCAL){
	...
	"SELECT	DISTINCT '1' AS QUANTIDADE,\
			...
			A.NUMEROMOV AS NUMERO_OS,\
			M.NOME AS LOCALIZACAO,\
			(SELECT TOP 1 NOMEFANTASIA FROM TITMMOV (NOLOCK), TPRODUTO (NOLOCK) WHERE TITMMOV.CODCOLIGADA = TPRODUTO.CODCOLPRD AND TITMMOV.IDPRD = TPRODUTO.IDPRD AND TPRODUTO.NOMEFANTASIA LIKE '%REVISAO%' AND TITMMOV.CODCOLIGADA = A.CODCOLIGADA AND TITMMOV.IDMOV = A.IDMOV) AS ITEM,\
			A.DATACRIACAO\
	...
	ORDER BY A.DATACRIACAO ASC, A.NUMEROMOV ASC";
